...
use std::sync::Arc;
use uuid::Uuid;
use crate::models::{StoredFile, FileMetadata};
...
pub async fn store_secret(client: &Client, secret: String, expiration: u64) -> Result<String, redis::RedisError> {
...
pub async fn store_file(client: &Client, metadata: FileMetadata, encrypted_data: String, expiration: u64) -> Result<String, redis::RedisError> {
    let mut conn = client.get_multiplexed_async_connection().await?;
...
pub async fn get_file(client: &Client, id: &str) -> Result<Option<StoredFile>, redis::RedisError> {
    ...
    if let Some(json_str) = result {
        let stored_file: StoredFile = serde_json::from_str(&json_str).map_err(|e| {
             redis::RedisError::from((redis::ErrorKind::TypeError, "Deserialization error", e.to_string()))
        })?;
